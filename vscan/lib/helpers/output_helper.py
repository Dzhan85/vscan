...
class output_helper(object):
    ...
    def output_json(self, filename):
        file = file_helper(filename)
        output = dict()
        output['Your Target'] = self.scanner.target
        output['Your Base Host'] = self.scanner.base_host
        output['Your Port'] = self.scanner.port
        output['Your Real Port'] = self.scanner.real_port
        output['Ignore HTTP Codes'] = self.scanner.ignore_http_codes
        output['Ignore Content Length'] = self.scanner.ignore_content_length
        output['Your Wordlist'] = self.scanner.wordlist
    ...
    def generate_header(self):
        output = "Result of Virtual-Host scanner Log: {} {}\n".format(
            time.strftime("%d/%m/%Y"),
            time.strftime("%H:%M:%S")
        )
